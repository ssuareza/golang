...
func (c *Client) GetBalance() (float64, error) {
	// set url
	balanceURL := fmt.Sprintf("%s%s%s%s", c.URL, "/v4/profiles/", profileId, "/balances?types=STANDARD")
...
func (c *Client) GetRate(source, target string) (float64, error) {
	// set url
	rateURL := fmt.Sprintf("%s%s%s%s%s", c.URL, "/v1/rates?source=", source, "&target=", target)
...
func (c *Client) GetCardTransactionsByRange(until, since string) (CardTransactions, error) {
	// set url
	cardTransactionsURL := fmt.Sprintf("%s%s%s%s%s%s%s%s", c.URL, "/v1/profiles/", profileId, "/activities?status=COMPLETED&until=", until, "&since=", since, "&size=100")
